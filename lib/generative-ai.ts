...
if (!API_KEY) {
    console.error("CRITICAL ERROR: Gemini API Key is missing! Expo did not load it from the .env file.");
} else {
...
export const analyzeBuildsphereImage = async (prompt: string, base64Image: string, mimeType: string) => {
    ...
    try {
        ...
        if (error.message?.includes("404")) {
            console.log("DEBUG: 404 detected. This API key might not have access to gemini-2.5-flash yet.");
        }
        throw error;
    }
};
